...
const getFreeModelOrder = async ({ storage }: Params): Promise<LLMModel[]> => {
  console.log("Getting model order...");
  const modelOrder = await storage.getItem<LLMModel[]>("sync:freeModelOrder");
  return modelOrder!;
};

export default getFreeModelOrder;
