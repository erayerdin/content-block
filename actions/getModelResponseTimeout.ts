...
const getModelResponseTimeout = async ({
  storage,
}: Params): Promise<number> => {
  console.log("Getting model response timeout...");
  const modelResponseTimeout = await storage.getItem<number>(
    "sync:modelResponseTimeout"
  );
  return modelResponseTimeout!;
};

export default getModelResponseTimeout;
